...
public class QuizProvider {
    ...
    public ArrayList<ArrayList<Integer>> assignNewQuiz() {
        ...
        for (int quiz : quizList) {
            ...
            } while (members.get(randMem).getNewQuizList().contains(quiz) || (result.get(randMem).size() >= members.get(randMem).getNewQuizList().size()));
    ...
    public ArrayList<ArrayList<Integer>> assignReviewQuiz() {
        ...
        for (int quiz : quizList) {
            ...
            do {
                randMem = (int) (Math.random() * memberCount);
            } while (members.get(randMem).getReviewQuizList().contains(quiz) || (result.get(randMem).size() >= members.get(randMem).getReviewQuizList().size()));
